...
async def get_repairman_list_test(
    db: Session = Depends(get_db),
):
    """Тестовий endpoint для списку ремонтників без авторизації"""
    try:
        repairmans = get_all_repairmans(db)
        
        formatted_repairmans = []
        for repairman in repairmans:
            formatted_repairmans.append({
                "id": repairman.id,
        ...
        return {
            "status": "success",
            "data": formatted_repairmans,
            "count": len(formatted_repairmans)
        }
