...
async def get_renovation_list(
    ...
    try:
        ...
        for renovation in renovations:
            renovation_dict = {
                "id": renovation.id,
                "lantern_id": renovation.lantern_id,
                "start_date": renovation.start_date.isoformat() if renovation.start_date else None,
                "end_date": renovation.end_date.isoformat() if renovation.end_date else None,
                "status": renovation.status,
                "cost": renovation.cost,
                "repairman_id": renovation.repairman_id,
                "description": renovation.description,
                "priority": renovation.priority,
                "date": renovation.start_date.strftime(DATETIME_FORMAT) if renovation.start_date else None
            }
            formatted_renovations.append(renovation_dict)

        return JSONResponse(
            content=formatted_renovations,
            headers={
...
def get_single_renovation(
    ...
    renovation_dict = {
        "id": renovation.id,
        "lantern_id": renovation.lantern_id,
        "lanternId": renovation.lantern_id,  # For frontend compatibility
        "start_date": renovation.start_date.isoformat() if renovation.start_date else None,
        "scheduledDate": renovation.start_date.isoformat() if renovation.start_date else None,  # For frontend
        "end_date": renovation.end_date.isoformat() if renovation.end_date else None,
        "completedDate": renovation.end_date.isoformat() if renovation.end_date else None,  # For frontend
        "status": renovation.status,
        "cost": renovation.cost,
        "repairman_id": renovation.repairman_id,
        "description": renovation.description,
...
def update_renovation_details(
    ...
    if renovation.start_date:
        formatted_renovation.date = renovation.start_date.strftime(DATETIME_FORMAT)
    else:
        formatted_renovation.date = None
