...
def update_renovation_in_db(
    ...
    if date:
        try:
            ...
            datetime_combined = datetime.combine(date_obj, renovation.start_date.time() if renovation.start_date else datetime.now().time())
            renovation.start_date = datetime_combined
        except ValueError:
            raise HTTPException(
                status_code=400,
    ...
    if time:
        try:
            ...
            datetime_combined = datetime.combine(
                renovation.start_date.date() if renovation.start_date else datetime.now().date(), time_obj.time()
            )
            renovation.start_date = datetime_combined
        except ValueError:
            raise HTTPException(
